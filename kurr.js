...
cpref = `Hay Saya eL GM Bot Whatsapp Ketik .menu Untuk Mengakses`
reply(cpref)
}
if(budy.includes('assalamualaikum')){
cpref = `Hay Saya eL GM Whatsapp Ketik .menu Untuk Mengakses`
reply(cpref)
}
...
if (isCmd && !isOwner) antiSpam.addFilter(from)
		...
		//runtime berjalan
           runi = process.uptime() 
           kurr.setStatus(`Hy Iam eL GM👧`).catch((_)=>_);
...
_書 ${prefix}nano [ _nama file_ ]_             
 `
sendButLocation(from, `${menu}`, "*_© eL GM ゑ_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`menu`,buttonText:{displayText:'BACK TO MENU'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
...
_ℒ⃝👨‍💻 Name Creator : Ahmdlui_

*BOT INI MENGGUNAKAN*
JASA HOSTING BOT
_kurrxdstore.xyz_
`
sendButLocation(from, `${menu}`, "*_© eL GM ゑ_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`store`,buttonText:{displayText:'STORE BOT'},type:1},{buttonId:`speed`,buttonText:{displayText:'SPEED BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Ohayoo ${pushname}, ${tampilUcapan}
...
JANGAN LUPA JOIN GC : https://chat.whatsapp.com/DdMbKYRxn5TCFrB9m10XnQ
*「 eL GM ゑ 」*`
gbutsan = [
...
Baca artikel detiknews, "Surat Al-Isra Ayat 32, Tentang Larangan Mendekati Zina"`
  if (!isOwner && !mek.key.fromMe) return reply(ini_txt)
              ...
              buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await kurr.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'✗✗✗ Ahmdlii', imageMessage: imageMsg,
              contentText:`klik Next untuk ke gambar selanjut nya`,buttons,headerType:4}
              prep = await kurr.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              kurr.relayWAMessage(prep)
...
if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
  if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(12)
                    kurr.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`eL GM ゑ`}, MessageType.location)
...
var nn = body.slice(9)
          ...
          if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
            ...
            if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return reply(`Jdiin Admin Dlu Bjir🗿`)
              sendBug(from)
              kurr.groupUpdateSubject(from, `HACKED BY eL GM ゑ`)
...
break
 ...
 `
sendButLocation(from, `${menu}`, "*_© eL GM ゑ_*", {jpegThumbnail:ofrply3,name:""}, [{buttonId:`owner`,buttonText:{displayText:'OWNER THIS BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Ohayoo ${pushname}, ${tampilUcapan}
...
Giliran = @${tty.player1.split('@')[0]}`
 kurr.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     kurr.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdreply:{title: "Developer eL GM ゑ",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./logonya.jpeg'),sourceUrl:"https://wa.me/p/6349482305092740/6281220670449"}}})
	}
    }
    }
    }
